...
public final class ComponentsC9IahpResponsesAllImagesContentApplicationJsonSchemaTests {
    ...
    public void testSerialize() {
        ComponentsC9IahpResponsesAllImagesContentApplicationJsonSchema model = new ComponentsC9IahpResponsesAllImagesContentApplicationJsonSchema().setImages(Arrays.asList(new Image().setName("asyntv").setType(ImageType.BASE).setDistribution(Distribution.OPEN_SUSE).setSlug("bqrvvbqv").setPublicProperty(false).setRegions(Arrays.asList(RegionSlug.NYC1, RegionSlug.FRA1, RegionSlug.AMS3, RegionSlug.AMS2)).setCreatedAt(OffsetDateTime.parse("2021-03-31T02:39:34Z")).setMinDiskSize(1717431509).setSizeGigabytes(87.35843F).setDescription("bn").setTags(Arrays.asList("tlpwqpjnxjkhtups", "yo")).setStatus(ImageStatus.NEW).setErrorMessage("iyxfwkztsmsfbevy"), new Image().setName("fhkqytkzta").setType(ImageType.CUSTOM).setDistribution(Distribution.FREE_BSD).setSlug("dgjfc").setPublicProperty(false).setRegions(Arrays.asList(RegionSlug.SFO3)).setCreatedAt(OffsetDateTime.parse("2021-05-10T22:19:32Z")).setMinDiskSize(186442409).setSizeGigabytes(47.283424F).setDescription("kzju").setTags(Arrays.asList("ajquzxpixhyoi", "nfdbgsoscienezf")).setStatus(ImageStatus.RETIRED).setErrorMessage("n"), new Image().setName("pjnrtekwwnthro").setType(ImageType.BACKUP).setDistribution(Distribution.GENTOO).setSlug("yiurztvktjh").setPublicProperty(false).setRegions(Arrays.asList(RegionSlug.SFO2)).setCreatedAt(OffsetDateTime.parse("2021-11-10T06:53:54Z")).setMinDiskSize(195067521).setSizeGigabytes(57.997356F).setDescription("gaghpuzxkpyehh").setTags(Arrays.asList("ldhg", "edzfzqiyuqhtdere")).setStatus(ImageStatus.RETIRED).setErrorMessage("a"))).setLinks(new PageLinks()).setMeta(new MetaMeta().setTotal(1956443482));
        model = BinaryData.fromObject(model).toObject(ComponentsC9IahpResponsesAllImagesContentApplicationJsonSchema.class);
        Assertions.assertEquals("asyntv", model.getImages().get(0).getName());
        Assertions.assertEquals(ImageType.BASE, model.getImages().get(0).getType());
        Assertions.assertEquals(Distribution.OPEN_SUSE, model.getImages().get(0).getDistribution());
        Assertions.assertEquals("bqrvvbqv", model.getImages().get(0).getSlug());
