...
import com.digitalocean.api.models.DatabaseClusterStatus;
...
public final class DatabaseClusterTests {
    ...
    public void testSerialize() {
        DatabaseCluster model = new DatabaseCluster().setName("nuzejgvkveb").setEngine(DatabaseClusterEngine.MYSQL).setVersion("zllrz").setNumNodes(817821495).setSize("mmdqgmihzpimc").setRegion("rh").setPrivateNetworkUuid("klogx").setTags(Arrays.asList("zarhzvqnsqktcmbj", "zzoslpkyb", "glwkzpgajsqj", "emqbmfuvqarwzxuq")).setConnection(new DatabaseClusterConnection()).setPrivateConnection(new DatabaseClusterPrivateConnection()).setMaintenanceWindow(new DatabaseClusterMaintenanceWindow().setDay("yqe").setHour("r")).setRules(Arrays.asList(new FirewallRule().setUuid("r").setClusterUuid("wnvz").setType(FirewallRuleType.IP_ADDR).setValue("n"), new FirewallRule().setUuid("sxvjabjqqaxuyvy").setClusterUuid("nudn").setType(FirewallRuleType.TAG).setValue("abhjx"), new FirewallRule().setUuid("euipmpvksmitnsq").setClusterUuid("l").setType(FirewallRuleType.DROPLET).setValue("lkrdpqgfhy"), new FirewallRule().setUuid("kkld").setClusterUuid("c").setType(FirewallRuleType.K8S).setValue("cmfcnrjajq")));
