...
public final class AssignDropletsByTagTests {
    ...
    public void testDeserialize() {
        ...
        Assertions.assertEquals(ForwardingRuleTargetProtocol.HTTPS, model.getForwardingRules().get(0).getTargetProtocol());
    ...
    public void testSerialize() {
        AssignDropletsByTag model = new AssignDropletsByTag().setTag("zflb").setRegion(RegionSlug.NYC2).setName("qj").setSizeUnit(352717547).setSize(LoadBalancerBaseSize.LB_MEDIUM).setAlgorithm(LoadBalancerBaseAlgorithm.ROUND_ROBIN).setForwardingRules(Arrays.asList(new ForwardingRule().setEntryProtocol(ForwardingRuleEntryProtocol.UDP).setEntryPort(2118535051).setTargetProtocol(ForwardingRuleTargetProtocol.HTTPS).setTargetPort(180591536).setCertificateId("senerr").setTlsPassthrough(true), new ForwardingRule().setEntryProtocol(ForwardingRuleEntryProtocol.HTTPS).setEntryPort(1361345675).setTargetProtocol(ForwardingRuleTargetProtocol.HTTP2).setTargetPort(1316425856).setCertificateId("psjnpck").setTlsPassthrough(true), new ForwardingRule().setEntryProtocol(ForwardingRuleEntryProtocol.TCP).setEntryPort(1433921158).setTargetProtocol(ForwardingRuleTargetProtocol.HTTPS).setTargetPort(660826018).setCertificateId("nbdawsaoplvvmnbk").setTlsPassthrough(true))).setHealthCheck(new HealthCheck().setProtocol(HealthCheckProtocol.HTTP).setPort(1183916531).setPath("dwokqx").setCheckIntervalSeconds(1057888225).setResponseTimeoutSeconds(443944483).setUnhealthyThreshold(1960915680).setHealthyThreshold(1563414212)).setStickySessions(new StickySessions().setType(StickySessionsType.NONE).setCookieName("cgrd").setCookieTtlSeconds(2058896611)).setRedirectHttpToHttps(true).setEnableProxyProtocol(true).setEnableBackendKeepalive(false).setDisableLetsEncryptDnsRecords(false);
