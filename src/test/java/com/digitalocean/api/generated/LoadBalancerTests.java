...
public final class LoadBalancerTests {
    ...
    public void testDeserialize() {
        LoadBalancer model = BinaryData.fromString("{\"tag\":\"ghpakbqyhlsorn\",\"name\":\"eq\",\"ip\":\"kncjmybn\",\"size_unit\":1495260868,\"size\":\"lb-small\",\"algorithm\":\"least_connections\",\"status\":\"active\",\"created_at\":\"2021-11-06T01:39:53Z\",\"forwarding_rules\":[{\"entry_protocol\":\"http\",\"entry_port\":1410061846,\"target_protocol\":\"http\",\"target_port\":1842044853,\"certificate_id\":\"mbwppxirxbkitzmn\",\"tls_passthrough\":true}],\"health_check\":{\"protocol\":\"http\",\"port\":1282222748,\"path\":\"tjlxsgcemegdzdvy\",\"check_interval_seconds\":93206066,\"response_timeout_seconds\":720382134,\"unhealthy_threshold\":1594038170,\"healthy_threshold\":1427059061},\"sticky_sessions\":{\"type\":\"none\",\"cookie_name\":\"ifnivlut\",\"cookie_ttl_seconds\":932860252},\"redirect_http_to_https\":false,\"enable_proxy_protocol\":true,\"enable_backend_keepalive\":true,\"disable_lets_encrypt_dns_records\":false}").toObject(LoadBalancer.class);
        ...
        Assertions.assertEquals(ForwardingRuleTargetProtocol.HTTP, model.getForwardingRules().get(0).getTargetProtocol());
    ...
    public void testSerialize() {
        LoadBalancer model = new LoadBalancer().setName("eq").setSizeUnit(1495260868).setSize(LoadBalancerBaseSize.LB_SMALL).setAlgorithm(LoadBalancerBaseAlgorithm.LEAST_CONNECTIONS).setForwardingRules(Arrays.asList(new ForwardingRule().setEntryProtocol(ForwardingRuleEntryProtocol.HTTP).setEntryPort(1410061846).setTargetProtocol(ForwardingRuleTargetProtocol.HTTP).setTargetPort(1842044853).setCertificateId("mbwppxirxbkitzmn").setTlsPassthrough(true))).setHealthCheck(new HealthCheck().setProtocol(HealthCheckProtocol.HTTP).setPort(1282222748).setPath("tjlxsgcemegdzdvy").setCheckIntervalSeconds(93206066).setResponseTimeoutSeconds(720382134).setUnhealthyThreshold(1594038170).setHealthyThreshold(1427059061)).setStickySessions(new StickySessions().setType(StickySessionsType.NONE).setCookieName("ifnivlut").setCookieTtlSeconds(932860252)).setRedirectHttpToHttps(false).setEnableProxyProtocol(true).setEnableBackendKeepalive(true).setDisableLetsEncryptDnsRecords(false).setTag("ghpakbqyhlsorn");
