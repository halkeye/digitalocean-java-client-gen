...
public final class AppServiceSpecTests {
    ...
    public void testSerialize() {
        AppServiceSpec model = new AppServiceSpec().setName("cubiipuipw").setGit(new AppsGitSourceSpec().setBranch("nmacj").setRepoCloneUrl("nizshqvcim")).setGithub(new AppsGithubSourceSpec().setBranch("fgmblrrilbywdxsm").setDeployOnPush(false).setRepo("rwfscjfnynszquj")).setGitlab(new AppsGitlabSourceSpec().setBranch("voqyt").setDeployOnPush(false).setRepo("wb")).setImage(new AppsImageSourceSpec().setRegistry("yavutpthjoxois").setRegistryType(AppsImageSourceSpecRegistryType.DOCR).setRepository("bpimlqoljxkcgxxl").setTag("ffgcvizqz")).setDockerfilePath("l").setBuildCommand("lyoupfgfbkju").setRunCommand("yhgk").setSourceDir("in").setEnvs(Arrays.asList(new AppVariableDefinition().setKey("wzf").setScope(AppVariableDefinitionScope.BUILD_TIME).setType(AppVariableDefinitionType.GENERAL).setValue("tlahbq"), new AppVariableDefinition().setKey("ctxtgzukxi").setScope(AppVariableDefinitionScope.UNSET).setType(AppVariableDefinitionType.GENERAL).setValue("qqqxhrnxrx"), new AppVariableDefinition().setKey("pjui").setScope(AppVariableDefinitionScope.RUNTIME).setType(AppVariableDefinitionType.GENERAL).setValue("dzf"))).setEnvironmentSlug("zivj").setLogDestinations(new AppLogDestinationDefinition().setName("rqttbajlkatnw").setPapertrail(new AppLogDestinationPapertrailSpec().setEndpoint("iopid")).setDatadog(new AppLogDestinationDatadogSpec().setEndpoint("fku").setApiKey("s")).setLogtail(new AppLogDestinationLogtailSpec().setToken("dmligovibrxk"))).setCors(new AppsCorsPolicy().setAllowOrigins(Arrays.asList(new AppsStringMatch().setExact("lnorwmdu").setPrefix("pklvxw").setRegex("gdxpg"))).setAllowMethods(Arrays.asList("hiszepnnbjcrxgib", "daxconfozauorsuk")).setAllowHeaders(Arrays.asList("bqplh", "vnuuepzl")).setExposeHeaders(Arrays.asList("wzsoldweyuqdunv")).setMaxAge("nrwrbiork").setAllowCredentials(false)).setHealthCheck(new AppServiceSpecHealthCheck().setFailureThreshold(681185824).setPort(4458522526150344918L).setHttpPath("nhxmsi").setInitialDelaySeconds(759547890).setPeriodSeconds(601340793).setSuccessThreshold(486711131).setTimeoutSeconds(1044112339)).setHttpPort(5098855864202607843L).setInternalPorts(Arrays.asList(322470867216638009L, 573812384778003954L, 5828391548888381928L)).setRoutes(Arrays.asList(new AppRouteSpec().setPath("fjchvc").setPreservePathPrefix(false))).setInstanceCount(3930113246959080815L).setInstanceSizeSlug(AppComponentInstanceBaseInstanceSizeSlug.PROFESSIONAL_XL);
