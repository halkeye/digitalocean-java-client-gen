...
public final class AssignDropletsByIDTests {
    ...
    public void testDeserialize() {
        ...
        Assertions.assertEquals(ForwardingRuleTargetProtocol.TCP, model.getForwardingRules().get(0).getTargetProtocol());
    ...
    public void testSerialize() {
        AssignDropletsByID model = new AssignDropletsByID().setDropletIds(Arrays.asList(1166306369, 388884988)).setRegion(RegionSlug.AMS3).setName("bxovuqo").setSizeUnit(1880027005).setSize(LoadBalancerBaseSize.LB_MEDIUM).setAlgorithm(LoadBalancerBaseAlgorithm.LEAST_CONNECTIONS).setForwardingRules(Arrays.asList(new ForwardingRule().setEntryProtocol(ForwardingRuleEntryProtocol.UDP).setEntryPort(1312894369).setTargetProtocol(ForwardingRuleTargetProtocol.TCP).setTargetPort(450292275).setCertificateId("btbw").setTlsPassthrough(false), new ForwardingRule().setEntryProtocol(ForwardingRuleEntryProtocol.HTTP2).setEntryPort(947626527).setTargetProtocol(ForwardingRuleTargetProtocol.HTTPS).setTargetPort(520409326).setCertificateId("sjdle").setTlsPassthrough(true))).setHealthCheck(new HealthCheck().setProtocol(HealthCheckProtocol.TCP).setPort(373958050).setPath("ubsfxip").setCheckIntervalSeconds(1753476541).setResponseTimeoutSeconds(1510631363).setUnhealthyThreshold(2138732877).setHealthyThreshold(1929084172)).setStickySessions(new StickySessions().setType(StickySessionsType.NONE).setCookieName("upfazusj").setCookieTtlSeconds(1748080211)).setRedirectHttpToHttps(false).setEnableProxyProtocol(false).setEnableBackendKeepalive(false).setDisableLetsEncryptDnsRecords(false);
