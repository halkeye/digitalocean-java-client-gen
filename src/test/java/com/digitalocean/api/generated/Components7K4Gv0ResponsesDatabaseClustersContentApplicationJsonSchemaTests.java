...
public final class Components7K4Gv0ResponsesDatabaseClustersContentApplicationJsonSchemaTests {
    ...
    public void testSerialize() {
        Components7K4Gv0ResponsesDatabaseClustersContentApplicationJsonSchema model = new Components7K4Gv0ResponsesDatabaseClustersContentApplicationJsonSchema().setDatabases(Arrays.asList(new DatabaseCluster().setName("outnpdctuhspfefy").setEngine(DatabaseClusterEngine.REDIS).setVersion("uyeuyl").setNumNodes(599363082).setSize("hmtybkcgsuthhll").setRegion("mwynefxexlfciatx").setPrivateNetworkUuid("mdskjhhxdlajfoxc").setTags(Arrays.asList("vslxlh", "a", "krmukmyjmkxett", "slojfkqidnqt")).setConnection(new DatabaseClusterConnection()).setPrivateConnection(new DatabaseClusterPrivateConnection()).setMaintenanceWindow(new DatabaseClusterMaintenanceWindow().setDay("ac").setHour("u")).setRules(Arrays.asList()), new DatabaseCluster().setName("xq").setEngine(DatabaseClusterEngine.REDIS).setVersion("oy").setNumNodes(1495517843).setSize("iylpck").setRegion("ewsedveskwxe").setPrivateNetworkUuid("fn").setTags(Arrays.asList("tmjtsghp", "c", "cp", "arpzeqacdldtzm")).setConnection(new DatabaseClusterConnection()).setPrivateConnection(new DatabaseClusterPrivateConnection()).setMaintenanceWindow(new DatabaseClusterMaintenanceWindow().setDay("sivmfogdrtbfcmk").setHour("ftsjcwj")).setRules(Arrays.asList()), new DatabaseCluster().setName("vifdxkecifhocjx").setEngine(DatabaseClusterEngine.PG).setVersion("ooz").setNumNodes(403548767).setSize("txvcm").setRegion("funlcpxxvi").setPrivateNetworkUuid("jgvrquvpyggl").setTags(Arrays.asList("rdcueljtiahx", "fqryarvsx", "qbglcjkaysp")).setConnection(new DatabaseClusterConnection()).setPrivateConnection(new DatabaseClusterPrivateConnection()).setMaintenanceWindow(new DatabaseClusterMaintenanceWindow().setDay("abzfivf").setHour("kpysthhzagjf")).setRules(Arrays.asList())));
        model = BinaryData.fromObject(model).toObject(Components7K4Gv0ResponsesDatabaseClustersContentApplicationJsonSchema.class);
        Assertions.assertEquals("outnpdctuhspfefy", model.getDatabases().get(0).getName());
        Assertions.assertEquals(DatabaseClusterEngine.REDIS, model.getDatabases().get(0).getEngine());
        Assertions.assertEquals("uyeuyl", model.getDatabases().get(0).getVersion());
        Assertions.assertEquals(599363082, model.getDatabases().get(0).getNumNodes());
