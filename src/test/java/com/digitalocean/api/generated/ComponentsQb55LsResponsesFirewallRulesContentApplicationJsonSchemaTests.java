...
public final class ComponentsQb55LsResponsesFirewallRulesContentApplicationJsonSchemaTests {
    ...
    public void testSerialize() {
        ComponentsQb55LsResponsesFirewallRulesContentApplicationJsonSchema model = new ComponentsQb55LsResponsesFirewallRulesContentApplicationJsonSchema().setRules(Arrays.asList(new FirewallRule().setUuid("vqchoadhrs").setClusterUuid("vzvsp").setType(FirewallRuleType.APP).setValue("dsrgfajg"), new FirewallRule().setUuid("ubklrxhj").setClusterUuid("tcetjdv").setType(FirewallRuleType.IP_ADDR).setValue("dieq"), new FirewallRule().setUuid("ruwdxvqzxoebwgj").setClusterUuid("ibanb").setType(FirewallRuleType.APP).setValue("pwtzvpakloz"), new FirewallRule().setUuid("rpejp").setClusterUuid("s").setType(FirewallRuleType.APP).setValue("bttt")));
        model = BinaryData.fromObject(model).toObject(ComponentsQb55LsResponsesFirewallRulesContentApplicationJsonSchema.class);
        Assertions.assertEquals("vqchoadhrs", model.getRules().get(0).getUuid());
        Assertions.assertEquals("vzvsp", model.getRules().get(0).getClusterUuid());
        Assertions.assertEquals(FirewallRuleType.APP, model.getRules().get(0).getType());
        Assertions.assertEquals("dsrgfajg", model.getRules().get(0).getValue());
