...
public final class Components15CinloResponsesDatabaseClusterContentApplicationJsonSchemaTests {
    ...
    public void testSerialize() {
        Components15CinloResponsesDatabaseClusterContentApplicationJsonSchema model = new Components15CinloResponsesDatabaseClusterContentApplicationJsonSchema().setDatabase(new DatabaseCluster().setName("qmzjqrbrpvnmdy").setEngine(DatabaseClusterEngine.REDIS).setVersion("bojtjppglaohoqkp").setNumNodes(8082327).setSize("nqjilaywk").setRegion("cwm").setPrivateNetworkUuid("mhxdqaolfy").setTags(Arrays.asList("kbjpjvlywlt", "fwobb")).setConnection(new DatabaseClusterConnection()).setPrivateConnection(new DatabaseClusterPrivateConnection()).setMaintenanceWindow(new DatabaseClusterMaintenanceWindow().setDay("eivmak").setHour("hysowljuxlkbect")).setRules(Arrays.asList(new FirewallRule().setUuid("lkzazmgok").setClusterUuid("gjqafkmkrokzr").setType(FirewallRuleType.K8S).setValue("qetwpqrtvaozn"), new FirewallRule().setUuid("iezeagm").setClusterUuid("it").setType(FirewallRuleType.IP_ADDR).setValue("gedhfpjstlzm"))));
        model = BinaryData.fromObject(model).toObject(Components15CinloResponsesDatabaseClusterContentApplicationJsonSchema.class);
        Assertions.assertEquals("qmzjqrbrpvnmdy", model.getDatabase().getName());
        Assertions.assertEquals(DatabaseClusterEngine.REDIS, model.getDatabase().getEngine());
        Assertions.assertEquals("bojtjppglaohoqkp", model.getDatabase().getVersion());
        Assertions.assertEquals(8082327, model.getDatabase().getNumNodes());
