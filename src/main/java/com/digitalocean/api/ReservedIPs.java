...
public final class ReservedIPs {
     ...
     */
     ReservedIPs(GeneratedClient client) {
        this.service = RestProxy.create(ReservedIPsService.class, client.getHttpPipeline(), client.getSerializerAdapter());
        this.client = client;
    }

    /**
     * The interface defining all the services for GeneratedClientReservedIPs to be used by the proxy service to
     * perform REST calls.
     */
    @Host("{$host}")
    ...
    private interface ReservedIPsService {
        ...
        @UnexpectedResponseExceptionType(ErrorException.class)
        Mono<ReservedIPsCreateResponse> create(@HostParam("$host") String host, @BodyParam("application/json") Object body, @HeaderParam("Accept") String accept);

        @Get("/v2/reserved_ips/{reserved_ip}")
        @ExpectedResponses({200, 401, 404, 429, 500})
        @UnexpectedResponseExceptionType(ErrorException.class)
        Mono<ReservedIPsGetResponse> get(@HostParam("$host") String host, @PathParam("reserved_ip") String reservedIp, @HeaderParam("Accept") String accept);

        @Delete("/v2/reserved_ips/{reserved_ip}")
        @ExpectedResponses({204, 401, 404, 429, 500})
        @UnexpectedResponseExceptionType(ErrorException.class)
        Mono<ReservedIPsDeleteResponse> delete(@HostParam("$host") String host, @PathParam("reserved_ip") String reservedIp, @HeaderParam("Accept") String accept);
    }
    ...
    public Mono<Object> getAsync(String reservedIp) {
        return getWithResponseAsync(reservedIp)
            .flatMap(res -> Mono.justOrEmpty(res.getValue()));}
    ...
    public Mono<Error> deleteAsync(String reservedIp) {
        return deleteWithResponseAsync(reservedIp)
            .flatMap(res -> Mono.justOrEmpty(res.getValue()));}
}
