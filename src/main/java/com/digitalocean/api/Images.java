...
public final class Images {
    ...
    /**
     * The interface defining all the services for GeneratedClientImages to be used by the proxy service to perform REST
     * calls.
     */
    @Host("{$host}")
    ...
    private interface ImagesService {
        ...
        Mono<ImagesListResponse> list(
                @HostParam("$host") String host,
                @QueryParam("type") Enum59 type,
                @QueryParam("private") Boolean privateParameter,
        ...
        @UnexpectedResponseExceptionType(ErrorException.class)
        Mono<ImagesCreateCustomResponse> createCustom(
                @HostParam("$host") String host,
                @BodyParam("application/json") ImageNewCustom body,
                @HeaderParam("Accept") String accept);

        @Get("/v2/images/{image_id}")
        @ExpectedResponses({200, 401, 404, 429, 500})
        @UnexpectedResponseExceptionType(ErrorException.class)
        Mono<ImagesGetResponse> get(
                @HostParam("$host") String host,
                @PathParam("image_id") Object imageId,
                @HeaderParam("Accept") String accept);

        @Put("/v2/images/{image_id}")
        @ExpectedResponses({200, 401, 404, 429, 500})
        @UnexpectedResponseExceptionType(ErrorException.class)
        Mono<ImagesUpdateResponse> update(
                @HostParam("$host") String host,
        ...
        @UnexpectedResponseExceptionType(ErrorException.class)
        Mono<ImagesDeleteResponse> delete(
                @HostParam("$host") String host,
                @PathParam("image_id") int imageId,
                @HeaderParam("Accept") String accept);
    }
     ...
     * List All Images
     *
     * <p>To list all of the images available on your account, send a GET request to /v2/images.
     *
     * <p>## Filtering Results -----
     *
     ...
     * <p>**Image Type**
     *
     * <p>Either 1-Click Application or OS Distribution images can be filtered by using the `type` query parameter.
     *
     * <p>&gt; Important: The `type` query parameter does not directly relate to the `type` attribute.
     *
     * <p>To retrieve only ***distribution*** images, include the `type` query parameter set to distribution,
     * `/v2/images?type=distribution`.
     *
     * <p>To retrieve only ***application*** images, include the `type` query parameter set to application,
     * `/v2/images?type=application`.
     *
     * <p>**User Images**
     *
     * <p>To retrieve only the private images of a user, include the `private` query parameter set to true,
     * `/v2/images?private=true`.
     *
     * <p>**Tags**
     *
     * <p>To list all images assigned to a specific tag, include the `tag_name` query parameter set to the name of the
     * tag in your GET request. For example, `/v2/images?tag_name=$TAG_NAME`.
     *
    ...
    public Mono<ImagesListResponse> listWithResponseAsync(
            Enum59 type, Boolean privateParameter, String tagName, Integer perPage, Integer page) {
     ...
     *
     * <p>To create a new custom image, send a POST request to /v2/images. The body must contain a url attribute
     * pointing to a Linux virtual machine image to be imported into DigitalOcean. The image must be in the raw, qcow2,
     * vhdx, vdi, or vmdk format. It may be compressed using gzip or bzip2 and must be smaller than 100 GB after being
     * decompressed.
     *
    ...
    public Mono<Object> createCustomAsync(ImageNewCustom body) {
        return createCustomWithResponseAsync(body).flatMap(res -> Mono.justOrEmpty(res.getValue()));
    }

    /**
     * Retrieve an Existing Image
     *
     * <p>To retrieve information about an image, send a `GET` request to `/v2/images/$IDENTIFIER`.
     *
     * @param imageId A unique number (id) or string (slug) used to identify and reference a specific image.
     *     <p>**Public** images can be identified by image `id` or `slug`.
     *     <p>**Private** images *must* be identified by image `id`.
     * @throws IllegalArgumentException thrown if parameters fail the validation.
     * @throws ErrorException thrown if the request is rejected by server.
     * @throws RuntimeException all other wrapped checked exceptions if the request fails to be sent.
     * @return the response body on successful completion of {@link Mono}.
    ...
    public Mono<Object> getAsync(Object imageId) {
        return getWithResponseAsync(imageId).flatMap(res -> Mono.justOrEmpty(res.getValue()));
    }

    /**
     * Update an Image
     *
     * <p>To update an image, send a `PUT` request to `/v2/images/$IMAGE_ID`. Set the `name` attribute to the new value
     * you would like to use. For custom images, the `description` and `distribution` attributes may also be updated.
     *
    ...
    public Mono<Object> updateAsync(int imageId, ImageUpdate body) {
        return updateWithResponseAsync(imageId, body).flatMap(res -> Mono.justOrEmpty(res.getValue()));
    }

    /**
     * Delete an Image
     *
     * <p>To delete a snapshot or custom image, send a `DELETE` request to `/v2/images/$IMAGE_ID`.
     *
     * @param imageId A unique number that can be used to identify and reference a specific image.
     * @throws IllegalArgumentException thrown if parameters fail the validation.
     * @throws ErrorException thrown if the request is rejected by server.
    ...
    public Mono<Error> deleteAsync(int imageId) {
        return deleteWithResponseAsync(imageId).flatMap(res -> Mono.justOrEmpty(res.getValue()));
    }
}
