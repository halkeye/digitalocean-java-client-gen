 ...
 * Database clusters) in order to better organize or facilitate the lookups and actions on it. Tags have two attributes:
 * a user defined `name` attribute and an embedded `resources` attribute with information about resources that have been
 * tagged.
 */
@Fluent
public final class Tags {
     ...
     * There is a limit of 255 characters per tag.
     *
     * <p>**Note:** Tag names are case stable, which means the capitalization you use when you first create a tag is
     * canonical.
     *
     * <p>When working with tags in the API, you must use the tag's canonical capitalization. For example, if you create
     * a tag named "PROD", the URL to add that tag to a resource would be
     * `https://api.digitalocean.com/v2/tags/PROD/resources` (not `/v2/tags/prod/resources`).
     *
     * <p>Tagged resources in the control panel will always display the canonical capitalization. For example, if you
     * create a tag named "PROD", you can tag resources in the control panel by entering "prod". The tag will still
     * display with its canonical capitalization, "PROD".
     *
