...
public final class TagsOperations {
    ...
    private interface TagsService {
        ...
        Mono<TagsListResponse> list(@HostParam("$host") String host, @QueryParam("per_page") Integer perPage, @QueryParam("page") Integer page, @HeaderParam("Accept") String accept);

        @Post("/v2/tags")
        @ExpectedResponses({201, 400, 401, 429, 500})
        @UnexpectedResponseExceptionType(ErrorException.class)
        Mono<TagsCreateResponse> create(@HostParam("$host") String host, @BodyParam("application/json") Tags body, @HeaderParam("Accept") String accept);

        @Get("/v2/tags/{tag_id}")
        @ExpectedResponses({200, 401, 404, 429, 500})
        @UnexpectedResponseExceptionType(ErrorException.class)
        Mono<TagsGetResponse> get(@HostParam("$host") String host, @PathParam("tag_id") String tagId, @HeaderParam("Accept") String accept);

        @Delete("/v2/tags/{tag_id}")
        @ExpectedResponses({204, 401, 404, 429, 500})
        @UnexpectedResponseExceptionType(ErrorException.class)
        Mono<TagsDeleteResponse> delete(@HostParam("$host") String host, @PathParam("tag_id") String tagId, @HeaderParam("Accept") String accept);

        @Post("/v2/tags/{tag_id}/resources")
        @ExpectedResponses({204, 401, 404, 429, 500})
        @UnexpectedResponseExceptionType(ErrorException.class)
        Mono<TagsAssignResourcesResponse> assignResources(@HostParam("$host") String host, @PathParam("tag_id") String tagId, @BodyParam("application/json") TagsResource body, @HeaderParam("Accept") String accept);

        @Delete("/v2/tags/{tag_id}/resources")
        @ExpectedResponses({204, 401, 404, 429, 500})
        @UnexpectedResponseExceptionType(ErrorException.class)
        Mono<TagsUnassignResourcesResponse> unassignResources(@HostParam("$host") String host, @PathParam("tag_id") String tagId, @BodyParam("application/json") TagsResource body, @HeaderParam("Accept") String accept);
    }
     ...
     * 
     * @param body A tag is a label that can be applied to a resource (currently Droplets, Images, Volumes, Volume Snapshots, and Database clusters) in order to better organize or facilitate the lookups and actions on it.
     ...
     * 
     * @param tagId The name of the tag. Tags may contain letters, numbers, colons, dashes, and underscores. There is a limit of 255 characters per tag.
     * @throws IllegalArgumentException thrown if parameters fail the validation.
     * @throws ErrorException thrown if the request is rejected by server.
     * @throws RuntimeException all other wrapped checked exceptions if the request fails to be sent.
     * @return the response body on successful completion of {@link Mono}.
    ...
    public Mono<Object> getAsync(String tagId) {
        return getWithResponseAsync(tagId)
            .flatMap(res -> Mono.justOrEmpty(res.getValue()));}
    ...
    public Mono<Error> deleteAsync(String tagId) {
        return deleteWithResponseAsync(tagId)
            .flatMap(res -> Mono.justOrEmpty(res.getValue()));}
    ...
    public Mono<Error> assignResourcesAsync(String tagId, TagsResource body) {
        return assignResourcesWithResponseAsync(tagId, body)
            .flatMap(res -> Mono.justOrEmpty(res.getValue()));}
    ...
    public Mono<Error> unassignResourcesAsync(String tagId, TagsResource body) {
        return unassignResourcesWithResponseAsync(tagId, body)
            .flatMap(res -> Mono.justOrEmpty(res.getValue()));}
}
