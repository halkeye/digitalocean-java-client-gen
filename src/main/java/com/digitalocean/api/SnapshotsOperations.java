...
public final class SnapshotsOperations {
    ...
    /**
     * The interface defining all the services for GeneratedClientSnapshotsOperations to be used by the proxy service
    ...
    private interface SnapshotsService {
        ...
        Mono<SnapshotsListResponse> list(@HostParam("$host") String host, @QueryParam("per_page") Integer perPage, @QueryParam("page") Integer page, @QueryParam("resource_type") Enum89 resourceType, @HeaderParam("Accept") String accept);

        @Get("/v2/snapshots/{snapshot_id}")
        @ExpectedResponses({200, 401, 404, 429, 500})
        @UnexpectedResponseExceptionType(ErrorException.class)
        Mono<SnapshotsGetResponse> get(@HostParam("$host") String host, @PathParam("snapshot_id") Object snapshotId, @HeaderParam("Accept") String accept);

        @Delete("/v2/snapshots/{snapshot_id}")
        @ExpectedResponses({204, 401, 404, 429, 500})
        @UnexpectedResponseExceptionType(ErrorException.class)
        Mono<SnapshotsDeleteResponse> delete(@HostParam("$host") String host, @PathParam("snapshot_id") Object snapshotId, @HeaderParam("Accept") String accept);
    }
    ...
    public Mono<Object> listAsync(Integer perPage, Integer page, Enum89 resourceType) {
        return listWithResponseAsync(perPage, page, resourceType)
    ...
    public Mono<Error> deleteAsync(Object snapshotId) {
        return deleteWithResponseAsync(snapshotId)
            .flatMap(res -> Mono.justOrEmpty(res.getValue()));}
}
